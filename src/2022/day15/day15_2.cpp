 ...
 * solutions as well to get some inspioration and findign new techniques etc.
 *
 * Probably based on the interval approach by Simon Toth
 *
 * Since I'm not running with the latest C++ standards so that std::ranges is
 * available I had some issues rewriting this. However after fixing how
 * Intervals are compared (only by comparing the min and not the entire
 * interval) it managed to get the correct answer for the puzzle
 */
#include <chrono>
#include <fstream>
#include <iostream>
